...
pub fn hardware_cpu_id() -> u8 {
    let hartid: usize;
    unsafe { asm!("mv {}, tp", out(reg) hartid, options(nomem, nostack, preserves_flags)) };
    hartid as u8
}
...
pub unsafe fn store_cpu_ptr(ptr: *mut super::Cpu) {
    unsafe { asm!("csrw sscratch, {}", in(reg) ptr as usize, options(nomem, nostack, preserves_flags)); }
}
